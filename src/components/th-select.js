...
export class ThSelect extends Component {
    ...
    _navigate(direction) {
        ...
        let idx = Array.from(options).findIndex((opt) => opt.classList.contains("th-select__option--highlighted"));
        if (idx === -1) {
            idx = Array.from(options).findIndex((opt) => opt.classList.contains("th-select__option--selected"));
