...
- `th-toast`: module-level `refs[]` array for auto-stacking with 55px gap; each toast is `position: fixed` with no container div; `close()` method dispatches `close` event; auto-removes after `duration` seconds (default 8); `Thyme.info/warn/error/success` shortcuts default to 8 seconds
